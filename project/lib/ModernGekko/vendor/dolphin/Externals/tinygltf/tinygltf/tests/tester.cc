...
TEST_CASE("image-uri-spaces", "[issue-236]") {
  ...
  // LOCAL01b: use a portable on-disk fixture name with repeated internal
  // spaces. A leading ASCII space can be lost during Windows ZIP extraction.
  tinygltf::Model model;
  bool ret = ctx.LoadASCIIFromFile(
      &model, &err, &warn,
  ...
  REQUIRE(model.images[0].uri == "2x2 image  has multiple      spaces.png");
