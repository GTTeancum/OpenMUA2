...
4. Optimize shared generated/native transfer paths that benefit MSVC and GCC/Clang together. Same-section REL translation is fast-pathed in both directions and empty forced-fallback scans are removed; the next source-level focus is the cached host-call chunk-state query and remaining cross-section/cross-chunk transfer overhead.
