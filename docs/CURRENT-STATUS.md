...
September 24, 2026. This file tracks the current reconstructed source on `main`; the older LOCAL01 recovery boundary is no longer an accurate description of the checked-in implementation.
...
4. Optimize shared generated/native transfer paths that benefit MSVC and GCC/Clang together. Same-section REL translation is fast-pathed in both directions, eligible continuation now reuses the generated linked result directly, empty forced-fallback scans are removed, cached host-call state is read directly, duplicate generated host-call chassis probes are removed, and the redundant module-active burst back-edge branch is gone. A concrete next source-level candidate is to test `ppc.downcount > 0` and CPU running state before `fast_native_continue()` at the native burst back-edge, avoiding one eligibility/REL/host-call probe when the burst must already terminate; preserve semantics with a focused regression before accepting it.
