...
4. Optimize shared generated/native transfer paths that benefit MSVC and GCC/Clang together. Same-section REL translation is now fast-pathed in both directions, so the next source-level focus should be the remaining per-block chassis checks and cross-section/cross-chunk transfer overhead, selected from fresh source inspection or profiling evidence.
