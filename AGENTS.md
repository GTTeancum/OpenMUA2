...
Testing exception: the user may upload their own copy of the RMSE52 WBFS into an agent session (including a remote/cloud session) **solely for testing**: building, extracting, booting, benchmarking and regression checks. An agent in such a session may ask for it when a game-level test is actually needed. Keep the uploaded image and everything derived from it under `.local` (or the session scratch area), verify it read-only, and never alter, stage, commit, push, attach to a PR/issue/release/artifact, or send it to any other service. Share only logs, hashes and evidence text, never game bytes. Delete the uploaded copy and derived game data when the session's testing ends. The user's original local WBFS remains the master copy.
